...
noise_chan = 3;
cancel_chans = [1 2 7 8];   % FL FR SL SR
...
% Add file names for more speakers (XX indicates date of IR)
ir_files = { ...
   'Spath_L_feb_09_1.wav', ...
   'Spath_R_feb_09_1.wav'  ...
};

for k = 1:K
    [s_raw,Fs] = audioread(ir_files{k});
...
y_buf_S = cell(K,1);
xF_delay_buf = cell(K,1);         % delay line per speaker

% Measured delay from speakers 
% (should be in order of speaker definition from above)
% L 2522 R 2400 BL 2909 BR 2450 SL 2542 SR 2376
Dsec = [2522 2400 2542 2376];  

for k = 1:K
    y_buf_S{k} = zeros(length(S_hat{k})-1, 1);
    xF_delay_buf{k} = zeros(Dsec(k),1);
end
    ...
    for k = 1:phase
        ...
        % Phase 2: ANC active
            ...
            for k2 = 1:K
                ...
                [y_frame(:,k2), x_buf(:,k2)] = filter(w(:,k2), 1, rm, x_buf(:,k2));
                y_frame = max(min(y_frame, ymax), -ymax);
            end

            % Predict how the reference propogates through the room using
            % S_hat (Speaker-Mic path)
            x_f = zeros(frame_size, K);
            for k2 = 1:K
                % Filter reference signal through secondary path estimate (short FIR tail)
                [x_f_tail, y_buf_S{k2}] = filter(S_hat{k2}, 1, rm, y_buf_S{k2});
            
                % Apply measured pure delay Dsec to the filtered-x signal
                xF_delay_buf{k2} = [xF_delay_buf{k2}(frame_size+1:end); x_f_tail];
                x_f(:,k2) = xF_delay_buf{k2}(1:frame_size);
            end
            
            % FxNLMS adaptation
            for n = 1:frame_size
                for k2 = 1:K
                    xhat_buf(:,k2) = [x_f(n,k2); xhat_buf(1:end-1,k2)];
                    denom = (xhat_buf(:,k2)'*xhat_buf(:,k2)) + delta;
                    w(:,k2) = (1 - leak)*w(:,k2) - (mu/denom) * em(n) * xhat_buf(:,k2);
                end
            end
        end
        ...
        if idx2 <= Nrec
            ...
            errLog(writeIdx:idx2)    = single(em);
        ...
        for k2 = 1:K
            y_out(:,cancel_chans(k2)) = y_frame(:,k2);
        end
