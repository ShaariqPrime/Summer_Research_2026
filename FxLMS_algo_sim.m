...
[p_raw,Fs] = audioread('Ppath_IR_SL_15_1.wav');
...
t = (0:length(p_raw)-1)/Fs;

figure; plot(t, p_raw);
xlabel('Time (s)'); ylabel('Amplitude');
title('Measured primary impulse response (REW export)');
grid on;
...
post = round(0.085 * Fs);   % 85 ms after peak
...
[s_raw,Fs] = audioread('Spath_IR_SL_15_1.wav');

% Quick check to see if files have been exported correctly (4800 samples)
fprintf('Loaded IR: %d samples, Fs = %d Hz, duration = %.3f s\n', ...
    length(s_raw), Fs, length(s_raw)/Fs);

t = (0:length(s_raw)-1)/Fs;

figure; plot(t, s_raw);
xlabel('Time (s)'); ylabel('Amplitude');
title('Measured secondary impulse response (REW export)');
grid on;
